...
class TemporalDataset(object):
    ...
    def eval(
        ...
        # if split == 'test':
        #     np.save('time_predict.npy', np.array(test[:, 3]))
        #     np.save('lhs_predict.npy', np.array(test[:, 0]))
        #     np.save('rel_predict.npy', np.array(test[:, 1]))
        #     lhs, rel, rhs, time, rel_2, time_phase, time_3 = model.get_tensor(test)
        #     lhs = lhs.detach().cpu().numpy()
        #     rel = rel.detach().cpu().numpy()
        #     rhs = rhs.detach().cpu().numpy()
        #     time = time.detach().cpu().numpy()
        #     rel_2 = rel_2.detach().cpu().numpy()
        #     time_phase = time_phase.detach().cpu().numpy()
        #     time_3 = time_3.detach().cpu().numpy()
        #
        #     np.save('lhs.npy', np.array(lhs))
        #     np.save('rel.npy', np.array(rel))
        #     np.save('rhs.npy', np.array(rhs))
        #     np.save('time.npy', np.array(time))
        #     np.save('time_phase.npy', np.array(time_phase))
        #     np.save('rel_2.npy', np.array(rel_2))
        #     np.save('time_3.npy', np.array(time_3))
        #     print('Save over!')
        #     return 0
